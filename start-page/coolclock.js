...
CoolClock.prototype = {
	...
	init: function(options) {
		...
		this.canvas.setAttribute("width",this.displayRadius*2*dpr);
		this.canvas.setAttribute("height",this.displayRadius*2*dpr);
	...
	drawTextAt: function(theText,x,y,skin,font) {
		if (!skin) skin = this.getSkin();
		this.ctx.save();
		this.ctx.font = font || skin.font || this.font;
	...
	timeText: function(hour,min,sec) {
		...
		return '' +
			(c.showAmPm ? ((hour%12)==0 ? 12 : (hour%12)) : this.lpad2(hour)) + ':' +
			this.lpad2(min) +
	...
	render: function(day,month,hour,min,sec) {
		// Get the skin
	...
	refreshDisplay: function() {
		...
		if (this.gmtOffset != null) {
			...
			this.render(offsetNow.getUTCDate(), offsetNow.getUTCMonth(), offsetNow.getUTCHours(),offsetNow.getUTCMinutes(),offsetNow.getUTCSeconds());
		}
		else {
			// Use local time
			this.render(now.getDate(), now.getMonth(), now.getHours(),now.getMinutes(),now.getSeconds());
		}
	},

	// Set timeout to trigger a tick in the future
	nextTick: function() {
                var cId = this.canvasId;
		this.tickTimeout = setTimeout(function(){
                    CoolClock.config.clockTracker[cId].tick();
                },this.tickDelay);
